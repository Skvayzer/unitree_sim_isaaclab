#!/usr/bin/env python3
"""Test multiple block layouts to find safe spacing for multi-block stacking.
Runs block 0 grasp with blocks 1,2 at various positions."""
...
parser.add_argument("--layout", type=str, default="behind",
                    choices=["behind", "spread10", "spread15"],
                    help="Block layout: behind=blocks behind hand, "
                         "spread10=10cm spacing, spread15=15cm spacing")
...
# Configure block positions based on layout
if args.layout == "behind":
    # Blocks 1,2 BEHIND the hand (more negative y than block 0)
    # Forearm extends from shoulder toward hand, not beyond it
    env_cfg.scene.block_0.init_state.pos = (0.321, -0.180, 0.819)  # standard
    env_cfg.scene.block_1.init_state.pos = (0.321, -0.240, 0.819)  # 6cm behind hand
    env_cfg.scene.block_2.init_state.pos = (0.321, -0.300, 0.819)  # 12cm behind hand
elif args.layout == "spread10":
    # 10cm spacing (blocks at ±10cm from center)
    env_cfg.scene.block_0.init_state.pos = (0.321, -0.180, 0.819)
    env_cfg.scene.block_1.init_state.pos = (0.321, -0.280, 0.819)
    env_cfg.scene.block_2.init_state.pos = (0.321, -0.080, 0.819)
elif args.layout == "spread15":
    # 15cm spacing
    env_cfg.scene.block_0.init_state.pos = (0.321, -0.180, 0.819)
    env_cfg.scene.block_1.init_state.pos = (0.321, -0.330, 0.819)
    env_cfg.scene.block_2.init_state.pos = (0.321, -0.030, 0.819)

print(f"Layout: {args.layout}")
print(f"Block 0: {env_cfg.scene.block_0.init_state.pos}")
print(f"Block 1: {env_cfg.scene.block_1.init_state.pos}")
print(f"Block 2: {env_cfg.scene.block_2.init_state.pos}")
...
total_episodes = 0
total_lifted = 0
num_target = 128  # fewer episodes for faster sweep

print(f"\nTarget episodes: {num_target}\n")
...
for step_i in range(num_target * 500):
    ...
    block_local = block_pos - env.scene.env_origins
...
print(f"\nRESULTS ({args.layout}): {total_episodes} episodes, block_0 lift={lr:.1%}")
env.close()
