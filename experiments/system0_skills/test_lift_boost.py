#!/usr/bin/env python3
"""Test: Multi-block ENV with lift height boost to clear adjacent blocks.
Uses direct trajectory (no SM) but applies the same SP boost to LIFT/TRANSPORT
phases. Tests if higher lift prevents arm collision with adjacent blocks."""
...
parser.add_argument("--lift_sp_boost", type=float, default=-0.2,
                    help="Extra SP for LIFT/TRANSPORT phases (more negative = higher)")
...
# Create trajectory with lift boost applied
traj = ParameterizedArmTrajectory(CFG, device, num_envs)

# Apply lift SP boost to LIFT end, TRANSPORT, and DESCEND_TO_PLACE start
boost = args.lift_sp_boost
print(f"Lift SP boost: {boost} (base SP=-0.3, total={-0.3 + boost:.1f})")
traj.end_joints[:, 3, 0] += boost    # LIFT end
traj.start_joints[:, 4, 0] += boost   # TRANSPORT start
traj.end_joints[:, 4, 0] += boost     # TRANSPORT end
traj.start_joints[:, 5, 0] += boost   # DESCEND_TO_PLACE start
...
print(f"\nMulti-block ENV + lift boost ({boost}), blocks at 6cm spacing, block_0 at y=-0.180")
