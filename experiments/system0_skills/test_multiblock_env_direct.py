#!/usr/bin/env python3
"""Quick test: Multi-block ENV with single-block-style direct trajectory.
Isolates whether the multi-block ENV itself degrades block 0 grasping,
or if it's the state machine logic."""
...
# Run eval — same logic as run_single_block_eval but on multi-block env
prev_action = torch.zeros(num_envs, 7, device=device)
block_was_lifted = torch.zeros(num_envs, dtype=torch.bool, device=device)
total_episodes = 0
total_lifted = 0
num_target = 256  # enough episodes

print(f"\nMulti-block ENV + direct trajectory (no SM), block_0 at y=-0.180")
...
for step_i in range(num_target * 500):
    ...
    # Track block_0 state (same as single-block but accessing block_0)
